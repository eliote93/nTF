...
IF (nTracerCntl%lGammaScat1) THEN
  DO tid = 1, nThread
    ...
    CALL Dmalloc(TrackingDat(tid)%Jout1g, 3, 4, nxy)
...
DO iz = myzb, myze
  DO ipin = 1, nxy
    CALL AllocgPinXS(CMFD_gPinXS(ipin, iz), ng, ngg, 4, GamGroupInfo%InScatRange)
  ENDDO
ENDDO
