...
USE SubChCoupling_mod,    ONLY: CodeName
...
REAL :: PEXIT, Tout, toutavg, tfmax1, tfmax, tdopd, tdoplmax, TdoplMax0
...
DO iter = 1, 1
  IF(lSimpleTH) THEN
    ...
    elseif (CodeName=='ESCOT') then
      tfmax = 0._8
      CALL ESCOT_TH(Core, ThInfo, nTracerCntl,ToutAvg, PE)
      nchannel=1
    else
      DO ixy = xyb, xye
        IF(.NOT. CoolantTH(ixy)%lfuel) CYCLE
    ...
    DO ixy = xyb, xye
      ...
      CALL SteadyFuelConduction(powlin, PowLv, Tfmax1, RelPower(1:nzth, ixy), PwShape, BurnUpDist,                          &
                                RhoU, FuelTH(ixy), ThVar, ThOpt, nTracerCntl, PE)
...
601 format(11x,"Max Tf=",f7.1,"C,  Avg Outlet Temp",f7.2,"C", ", Dop. Change=",1p,e9.2,l2,i3)
