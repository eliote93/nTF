...
#include <Depletion.h>
#include <DefDBG.h>
!--- CNJ Edit : Depletion Modules with Intel MKL
#ifdef __INTEL_MKL
...
MODULE MKL_DEPL
  ...
  USE MKL_3D, ONLY : mklCntl, mklDepl
  IMPLICIT NONE
  ...
  nSubStep = mklDepl%nSubStep; SysByte = mklDepl%SysByte; Scale = mklDepl%Scale
#ifdef GDQUAD_SIMPLE
  nSubStep = nSubStep*2
#endif
  ...
  ALLOCATE(DeplFxrBundle%IdTrueDepl(ndnet))
...
!  CALL MacXsBase(XSMac, temp, niso, idiso, pnum, 1, ng, ng, 1._8, FALSE, TRUE)
...
!  IsoXsMacA => XsMac%IsoXsMacA
  ...
  DO i = 1, niso
    ...
    !xs1g(RctIdCap,j) = sigcap; xs1g(RctIdCapm,j) = sigcapm
    !xs1g(RctIdN2N,j) = sign2n; xs1g(RctIdN2Nm,j) = sign2nm
    IF (.NOT. DeplLib%lActinide(j)) THEN
      sigNA = DeplLib%RctXs(RctIdNA,j); sigNP = DeplLib%RctXs(RctIdNP,j)
      !xs1g(RctIdNA,j) = sigNA; xs1g(RctIdNP,j) = sigNP
    END IF
...
!  CALL FreeXsIsoMac(XsMac)
  ...
  !lHighOd = .TRUE.
  !WRITE(97,*) 'Step', DeplCntl%NowStep
  ...
  DO i = 1, NBun
    ...
    Tb = nTracer_dclock(FALSE, FALSE)
    CALL SetDeplSys(DeplLib, DeplFxrBundle, NsysB, ifxrbeg, DeplSysBundle, lGd, nDeplTh)
    Te = nTracer_dclock(FALSE, FALSE)
    TimeChk%DeplSysTime = TimeChk%DeplSysTime + (Te-Tb)
    TimeChk%DeplSolTime = TimeChk%DeplSolTime - (Te-Tb)
    CALL SolveDeplSys(DeplSysBundle, DeplFxrBundle, lCorrector, lGd, DeplSolTyp, nDeplTh)
    Tb = nTracer_dclock(FALSE, FALSE)
  ...
  !lQuad = .FALSE.
  !print*, lQuad, lCorrector, DeplFxrBundle%nTrueGd
#ifdef GDQUAD_SIMPLE
  ...
  IF (lQuad .AND. lCorrector .AND. (DeplFxrBundle%nTrueGd.GT.0)) THEN
    ...
    DO i = 1, NBun
      ...
      DO j = 1, nSubStep
        ...
        !END IF
        ! ------------------------------------------ 1st --------------------------------------------
        CALL SetDeplSys(DeplLib, DeplFxrBundle, NsysB, ifxrbeg, DeplSysBundle, lGd, nDeplTh, nSubStep)
        ...
        DO k = 1, NsysB
          pnums_Gd155(k,2) = DeplSysBundle%pnums1(NofIso*(k-1)+i155)
          pnums_Gd155(k,2) = 0.5*(pnums_Gd155(k,2)+pnums_Gd155(k,1))
        END DO
        CALL DestroySys(DeplSysBundle)
        ...
        pnums_RK(:,2) = DeplSysBundle%pnums1(:)
        DO k = 1, NsysB
          pnums_Gd155(k,3) = DeplSysBundle%pnums1(NofIso*(k-1)+i155)
          pnums_Gd155(k,3) = 0.5*(pnums_Gd155(k,3)+pnums_Gd155(k,1))
        END DO
        CALL DestroySys(DeplSysBundle)
        ...
        pnums_RK(:,3) = DeplSysBundle%pnums1(:)
        DO k = 1, NsysB
          pnums_Gd155(k,4) = DeplSysBundle%pnums1(NofIso*(k-1)+i155)
        END DO
        CALL DestroySys(DeplSysBundle)
        ...
        pnums_RK(:,4) = DeplSysBundle%pnums1(:)
...
!      CALL CP_CA(pnum, 0._8, ntiso)
  ...
  SUBROUTINE Depletion_Driver_HP()
  USE PARAM
...
#endif
#ifdef ITER_CHK
  USE MatExponential, ONLY : minCGIter, minGSIter, minHmRank, maxCGIter, maxGSIter, maxHmRank
#endif
...
#ifdef ITER_CHK
    minCGIter = 100; minGSIter = 100; minHmRank = 611;
    maxCGIter = 0; maxGSIter = 0; maxHmRank = 0;
#endif
    !Solve Depletion Chain
    CALL DepletionStep(Core, FmInfo, ThInfo, GroupInfo, nTracerCntl, PE, .TRUE.)
#ifdef ITER_CHK
    WRITE(135,'(A)') "Pre-step"
    WRITE(135,'(A,I3,A,I3)') "   Min. Iter. CG =", minCGIter, "   Max. Iter. CG =", maxCGIter
    WRITE(135,'(A,I3,A,I3)') "   Min. Iter. GS =", minGSIter, "   Max. Iter. GS =", maxGSIter
    WRITE(135,'(A,I3,A,I3)') "   Min. Rank Hm =", minHmRank, "   Max. Iter. Hm =", maxHmRank
#endif
...
!#define HGDBG
...
#ifdef ITER_CHK
    WRITE(135,'(A,I3,A)') "Predictor, ",iBurnUp,"-th Step"
    WRITE(135,'(A,I3,A,I3)') "   Min. Iter. CG =", minCGIter, "   Max. Iter. CG =", maxCGIter
    WRITE(135,'(A,I3,A,I3)') "   Min. Iter. GS =", minGSIter, "   Max. Iter. GS =", maxGSIter
    WRITE(135,'(A,I3,A,I3)') "   Min. Rank Hm =", minHmRank, "   Max. Iter. Hm =", maxHmRank
#endif
    ...
    Tbeg = nTracer_dclock(FALSE, FALSE)
#ifdef HGDBG
    WRITE(120,*) 'Step',iBurnUp,'Corrector',DeplCntl%lPredict
#endif
#ifdef ITER_CHK
    minCGIter = 100; minGSIter = 100; minHmRank = 611;
    maxCGIter = 0; maxGSIter = 0; maxHmRank = 0;
    ...
    CALL DepletionStep(Core, FmInfo, ThInfo, GroupInfo, nTracerCntl, PE, .FALSE.)
#ifdef ITER_CHK
    WRITE(135,'(A,I3,A)') "Corrector, ",iBurnUp,"-th Step"
    WRITE(135,'(A,I3,A,I3)') "   Min. Iter. CG =", minCGIter, "   Max. Iter. CG =", maxCGIter
    WRITE(135,'(A,I3,A,I3)') "   Min. Iter. GS =", minGSIter, "   Max. Iter. GS =", maxGSIter
    WRITE(135,'(A,I3,A,I3)') "   Min. Rank Hm =", minHmRank, "   Max. Iter. Hm =", maxHmRank
#endif
