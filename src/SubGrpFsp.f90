...
INTEGER :: nfsr, nfxr, myzb, myze, iz, ilv, nlv
...
DO iz = myzb, myze
  ...
  DO ig = ig1, ig2
    IF(Core%lFuelPlane(iz)) THEN
      CALL UpdtFnAdj(Core, Fxr, ig, iz, PE)         
      ...
      DO ilv = 1, nlv
        ...
        DO iter = 1, itermax 
          ...
          niter = niter + 1
          IF(RTMASTER) THEN
            CALL EquipXSGen_MLG(Phis1g, SigLamPot, xstr1g, ilv, Core, Fxr, iz, ig, PE)
            CALL UpdtFtAdj(Core, Fxr, ilv, ig, iz, PE)
          ENDIF
          IF(errmaxlv .lt. epsm3) EXIT
          IF(RTMASTER) THEN
            CALL SetPlnLsigP_MLG(SigLamPot, xstr1g, lv, Core, Fxr, ilv, iz, ig, PE) 
...
#ifdef MPI_ENV
    CALL MPI_SYNC(PE%MPI_NTRACER_COMM)
    CALL REDUCE(niter, itersum, PE%MPI_NTRACER_COMM, .FALSE.)
#endif
    WRITE(mesg,'(2x,2(a,i3),a,i5,1p,e13.3)') '[Fuel] Pln', iz, ' Group ',ig,'  In_itr', itersum, errmax
    TimeChk%SubGrpRTniter = TimeChk%SubGrpRTniter + niter
    IF(master .AND. .NOT. lSilent) CALL MESSAGE(io8, TRUE, TRUE, mesg)
  ENDDO
  ...
  CALL REDUCE(niter, itersum, PE%MPI_NTRACER_COMM, .FALSE.)
#endif
  WRITE(mesg,'(2x,a,i3,a,i5,1p,e13.3)') '[Clad] Pln', iz, '  In_itr', itersum, errmax
  TimeChk%SubGrpRTniter = TimeChk%SubGrpRTniter + niter
  IF(master .AND. .NOT. lSilent) CALL MESSAGE(io8, TRUE, TRUE, mesg)
  ...
  CALL REDUCE(niter, itersum, PE%MPI_NTRACER_COMM, .FALSE.)
#endif
  WRITE(mesg,'(2x,a,i3,a,i5,1p,e13.3)') '[AIC]  Pln', iz, '  In_itr', itersum, errmax
  TimeChk%SubGrpRTniter = TimeChk%SubGrpRTniter + niter
  IF(master .AND. .NOT. lSilent) CALL MESSAGE(io8, TRUE, TRUE, mesg)
ENDDO ! DO iz = myzb, myze  
...
DO iz = myzb, myze
  ...
  !transport xs and source setup for Subgroup Fixed Source Problem
  IF(RTMASTER) THEN
    CALL SetPlnLsigP_Dancoff(SigLamPot, xstr1g, Core, Fxr, iz, PE)
    CALL SetSubGrpSrc1g(src1g, SigLamPot, xstr1g, Core, Fxr, iz, PE)
  ENDIF
  DO iter = 1, 100 
    !Solving FSP
...
INTEGER, POINTER :: idiso(:)
REAL :: TempAvgsq, Tempsq, siglp, ind, micsigb, micsig0, sigbsq, xdat(20), ydat(20), I_reg, I_avg, var(200)
REAL, POINTER :: pnum(:)
...
DO ipin = 1, nxy
  ...
  if (CellInfo(icel)%lAIC) CYCLE
  TempAvgsq = dsqrt(GetPinFuelTemp(Core, Fxr, iz, ipin))
  Npin = 0._8; pinareasum = 0._8
  NresPinAvg(ipin) = 0._8
  DO j = 1, nLocalFxr
    ifxr = FxrIdxSt + j - 1
    myFxr => Fxr(ifxr, iz)  
    if (.not.myFxr%lres) cycle
...
INTEGER :: niso, nisotot, iso, nid, idx, idisotot(nelthel)
INTEGER, POINTER :: idiso(:)
...
DO iz = myzb, myze
  DO ipin = xyb, xye
      ...
      DO j =1, nLocalFxr
        ...
        IF( .NOT. myFxr%lres ) CYCLE    
        niso = myFxr%niso
        CALL CoreResIsoUpdate(myFXR%idiso(1:niso), niso, iz)
        IF ( myFxr%lCLD ) CYCLE   
