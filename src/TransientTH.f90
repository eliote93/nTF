...
REAL :: TBulk, Tsurf, Tout, ToutAvg, Tfcl_max, Tfavg_max, htcoeff
...
nchannel = 0; ToutAvg = 0; Tfcl_max = 0; Tfavg_max = 0
DO ixy = 1, nxy
  ...
  DO iz = 1, ThVar%nzth
    ...
    DO iter = 1, 5 
      ...
      ELSE
        CALL tfcaltr(FuelTh(ixy)%tfvol(1:nr-3, iz), FuelTh(ixy)%tfuel(1:nr, iz), FuelTh(ixy)%tfueld(1:nr, iz), Tbulk, htcoeff, qf, qfd, nr, ThVar, ThOpt)
      ENDIF
      Tsurf =  FuelTh(ixy)%tfuel(nsurf, iz)
      qflux = htcoeff * (Tsurf - Tbulk)     !Heat Flux from Clad to Coolant
      qeff = qflux * ThVar%zetap; qeffd = CoolantTH(ixy)%qeffd(iz)
...
ThInfo%Tfcl_max = Tfcl_max; ThInfo%Tfavg_max = Tfavg_max
